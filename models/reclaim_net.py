...
from . import reclaim_net_conv_4, reclaim_net_resnet
...
class ReClaimNet(nn.Module):
    ...
    def __init__(self,way=None,shots=None,resnet=False):
        ...
        if resnet:
            ...
            self.resolution = 5 * 5
            self.feature_extractor = reclaim_net_resnet.resnet12()
            ...
            self.feature_extractor = reclaim_net_conv_4.BackBone()
            self.dim = self.num_channel*5*5
        ...
        n = 9
        if n > 1:
            self.feature_extractor = nn.DataParallel(self.feature_extractor, device_ids=[0,1,2,3])
