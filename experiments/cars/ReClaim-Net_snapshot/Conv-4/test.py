...
import yaml
sys.path.append('../../../../')
from models.reclaim_net_snapshot import ReClaimNet
...
model = ReClaimNet(resnet=False)
model.cuda()
models = [copy.deepcopy(model) for i in range(stage)]

[models[i].load_state_dict(torch.load(str(epoch*(i+1))+'_'+'2model_Conv-4.pth')) for i in range(stage)]
